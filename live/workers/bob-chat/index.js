 ...
 *
 * One model: qwen36-q4kxl on `gpu-01`, the lab's own GPU. Bedrock and Claude Haiku
...
// qwen36-q4kxl since 2026-09-24 — Qwen3.6-35B-A3B, Unsloth's UD-Q4_K_XL GGUF,
// imported locally like its predecessor (see hosts/gpu-01 in nixos-iac). Same
// architecture and footprint as qwen35-q4kl (Qwen3.5-35B-A3B, UD-Q4_K_L,
// 2026-09-08 to 2026-09-24), so the swap is a name change: ~20.8 GB of weights
// on the three cards, same prompt, same options.
//
// Chosen on two offline benches of this Worker's own prompt (18 probes, 4 then
// 6 trials each, every answer read by hand): about 7 clear errors in 180
// answers against about 14 for qwen35-q4kl, whose failures were mostly counts
// ("nine machines", "dix-neuf machines") and one "on utilise Proxmox". Median
// latency 1.0 s against 1.1 s. A trend, not proof — 14 against 7 is within
// what chance can do — but it never lost on anything that was measured.
//
// Two known habits: it answers "What does gpu-01 run on?" in French first, every
// time, and the language guard below retries it (~1.5 s); and it does not
// connect the gpu-02 article to Frigate unless the question names the GPU.
//
// Ollama's official qwen3.6:35b-a3b was benched too and set aside: it ships
// vision (+1.1 GB), its own sampling (temperature 1, presence_penalty 1.5),
// and invented more than this import.
const LOCAL_MODEL = "qwen36-q4kxl";
/**
 ...
 *
 * DEPUIS LE 2026-09-17, CE TEXTE EST UN REPLI. La version qui roule est la
 * section « Chat » de src/data/gpu-01-persona.md dans labodeludo.dev, publiée
 * par /gpu-01-grounding.json comme `persona` et lue dans buildSystem. Ce fichier
 * n'est plus l'endroit où changer la voix de gpu-01.
 *
...
/** True when the caller is the nightly job rather than a visitor.
 *
 * Same secret as /api/gpu-01/popular, deliberately: one token for one job, and it
 * already travels to this origin over TLS. It buys no privilege here — the
 * answer, the rate limit and the grounding are identical — it only says "this
 * question is a check, not something somebody wanted to know".
 */
function probeToken(request, env) {
...
/**
 * slug -> who signed it, read from the grounding's index by column NAME.
 *
 * The site publishes the author with each index line (kind|slug|date|author|…)
 * and nothing else: a grounding published before that column existed yields an
 * empty map, and the excerpts simply go out unsigned, exactly as before.
 */
function authorsBySlug(g) {
  const cols = String(g?.corpusFields || "").split("|");
  const a = cols.indexOf("author");
  const s = cols.indexOf("slug");
  const map = new Map();
  if (a < 0 || s < 0) return map;
  for (const line of String(g.corpus || "").split("\n")) {
    const f = line.split("|");
    if (f[s] && f[a] && !map.has(f[s])) map.set(f[s], f[a]);
  }
  return map;
}

// HIS OWN POST, when it answers. Asked « as-tu un article là-dessus » after
// the bge-m3 question, gpu-01 named Ludo's introduction and a 2026-08 post, never
// his own — the one with the detailed section. The signature (#38) settles
// whose « je » an excerpt is; it did not make him PREFER his own. Measured
// offline against qwen35-q4kl, four trials per turn, with the site's text fix
// (labodeludo.dev #220) already in the index so his section leads retrieval:
// signatures alone 1/4 on the question and 1/4 on the follow-up; with these
// two lines 4/4 and 4/4. Without the text fix his section is not retrieved
// and the rule has nothing to point at (0/4, 1/4) — both halves are needed.
// « et qu'elle répond » is not decoration: a control on a Ludo-only topic
// (four trials) forced no gpu-01-signed post into the answer.
const OWN_POST_POINTER_EN = "If one of these is signed gpu-01 and it answers, it is yours: give it, and add another author's if it answers too.\n";
const OWN_POST_POINTER_FR = "Si une de ces publications est signée gpu-01 et qu'elle répond, c'est la tienne : donne-la, et ajoute celle d'un autre auteur si elle répond aussi.\n";
const OWN_POST_TAIL_EN = "When an excerpt signed gpu-01 answers the question, that is your post: give its link, even if another author's excerpt answers too.\n";
const OWN_POST_TAIL_FR = "Quand un extrait signé gpu-01 répond à la question, c'est ton article : donne son lien, même si un extrait d'un autre auteur répond aussi.\n";

function excerptsMessage(hits, english, cited = [], pointer = false, authors = new Map()) {
  ...
  const body = hits
    .map((h, n) => {
      // WHO SIGNED IT, next to the date and for the same kind of reason. Every
      // excerpt went out as "your own posts", so Ludo's first person read as
      // gpu-01's: « le nom de mon chien », lifted from Ludo's article introducing
      // gpu-01, is Ludo's dog in Ludo's sentence. The signature settles whose
      // « je » it is without a rule in the persona to argue for it.
      const by = authors.get(h.slug);
      const sig = by ? (english ? `, by ${by}` : `, par ${by}`) : "";
      const head = `[${n + 1}] ${h.title}${h.date || sig ? ` (${h.date ? `publié le ${h.date}` : ""}${sig})` : ""} — ${h.url}`;
  ...
  const head = english
    ? "=== EXCERPTS FROM THIS SITE'S POSTS (retrieved for this question) ===\n" +
      "These passages come from the site's publications, each signed by its " +
      "author. They were picked by " +
    ...
    : "=== EXTRAITS DES ARTICLES DU SITE (récupérés pour cette question) ===\n" +
      "Ces passages viennent des publications du site, chacune signée par son " +
      "auteur. Ils ont été choisis " +
...
// Widened 2026-09-15. The original list was function words a French speaker
// would never write, and it worked — until a question arrived carrying none of
// them at all: "Can I run a local LLM on a $300 card?" scores zero to zero, the
// tie falls to French, and gpu-01 answered a plainly English question in French.
// The comment above already described this failure for follow-ups and solved it
// by inheriting from the conversation; a FIRST question has nothing to inherit.
//
// Every word added is English-only. A word that is also French — a, on, son,
// car, pour, en, ces, ses — is deliberately absent: it would move FRENCH
// questions, which is this same bug pointing the other way. Measured over 44
// real questions (the live candidate list, the published panel, and this
// session's generations): three move, all three from French to English, all
// three genuinely English, and not one French question moves.
const EN_WORDS = /\b(the|and|you|is|are|that|this|with|for|it|he|she|got|there|was|but|of|to|my|your|not|what|do|did|how|why|where|which|can|could|should|would|will|shall|does|doesn't|don't|didn't|have|has|had|been|being|were|am|be|i|me|we|us|they|them|his|her|him|their|our|its|who|whose|whom|when|from|about|into|over|under|between|because|if|or|than|then|these|those|such|any|every|each|both|other|another|while|after|before|during|through|without|within|against|upon|onto|still|just|also|only|very|too|more|most|less|least|much|many)\b/g;
...
function buildSystem(g) {
  return (
    // Who gpu-01 is comes from the SITE now, not from here. src/data/gpu-01-persona.md
    // in labodeludo.dev is the one file allowed to describe him, and the
    // grounding publishes its Chat section as `persona` — byte-identical to
    // the constant above on the day this landed, so the switch changed
    // nothing. The constant stays as the fallback for a grounding published
    // before the field existed, and as the copy a reviewer can read next to
    // the code. Editing it here no longer changes what runs: edit the file.
    `${g.persona || PERSONA}\n\n` +
...
export default {
  async fetch(request, env, ctx) {
    ...
    //
    // NOT counted when the caller holds POPULAR_TOKEN. That is the nightly job
    // verifying, through check-answerable.py, that a suggested question still
    // gets an answer — and counting those closes a loop that would quietly
    // destroy the thing this tally is for. A question verified every morning
    // gains a count every morning, so it climbs the popularity list on the
    // job's own traffic and keeps re-selecting itself. Worse once the job also
    // GENERATES questions: a question gpu-01 invented would be asked by the gate,
    // counted as if a visitor had typed it, and published the next day by
    // check-openers.py as a real one — laundering invented text through the
    // verbatim rule that is the whole trust boundary here.
    const probe = probeToken(request, env);
    if (env.BOB_QUESTIONS && !probe) {
      ctx.waitUntil(recordQuestion(env.BOB_QUESTIONS, last.content));
    }
    ...
    let excerpts;
    {
      ...
      try {
        ...
        if (hits.length) {
          excerpts = excerptsMessage(hits, english, citedBefore(messages), pointer, authorsBySlug(grounding));
        }
      } catch {
