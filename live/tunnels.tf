...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "k3s" {
  ...
  config = {
    ingress = [
      ...
      # There is no /api/embed path rule any more, removed 2026-09-15. It sent
      # embeddings to a cluster pod while that pod held gpu-01's third GPU; the
      # cluster now has its own GPU node (vm-03), so the card went back to
      # ollama and bge-m3 with it. The catch-all below serves both models.
      #
      # If a path split ever returns, it must sit BEFORE the catch-all: the
      # tunnel takes the first matching rule, so a path rule placed after it is
      # never reached. And route by PATH rather than by a new hostname — same
      # name means the same Access applications and the same WAF rule, while a
      # second hostname is a second public surface to gate, and gating is the
      # part that gets forgotten.
      #
      # Everything on this hostname is the host's Ollama, dialled straight
