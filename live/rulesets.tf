  ...
  rules = [
    {
      ...
      ref         = "00000000000000000000000000000080"
    },
    # ollama.pub.example.com fronts a raw Ollama server on gpu-01, and that API is not
    # read-only — /api/pull, /api/create and /api/delete all live there. Access
    # gates the host, but this blocks everything except the one method+path the
    # gpu-01-chat Worker uses, so a leaked service token buys an attacker inference
    # and nothing else. Two independent gates on purpose: Access is the
    # authentication, this is the authorization.
    {
      action = "block"
      action_parameters = {
      }
      description = "ollama.pub.example.com: allow only POST /api/chat and /api/embed"
      enabled     = true
      expression  = "http.host eq \"ollama.pub.example.com\" and not (http.request.method eq \"POST\" and (http.request.uri.path eq \"/api/chat\" or http.request.uri.path eq \"/api/embed\"))"
    },
  ]
